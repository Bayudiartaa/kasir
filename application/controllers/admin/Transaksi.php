...
class Transaksi extends CI_Controller
{
  ...
  function simpan_transaksi()
  {
    ...
    $data = $this->db->get_where('menu', ['kode_menu' => $kode_menu])->row_array();
    $harga_jual = $data['harga_jual'];

    $total_harga = $jumlah_item * $harga_jual;
   
    $a = $this->db->query("SELECT id_master_jual FROM master_jual ORDER BY id_master_jual DESC")->row_array();
    if ($a) {
    ...
    if ($no_faktur == "") {
      ...
      $this->db->insert('master_jual', $data);
    } else {
      echo  $nofak1 = $no_faktur;

      $h = $this->db->get_where('master_jual', ['nofak_jual' => $no_faktur])->row_array();
  ...
  {
    $nofak_jual = $this->input->post('nofak_jual');
    $h = $this->db->get_where('master_jual', ['nofak_jual' => $nofak_jual])->row_array();
  ...
  function transaksi_hapus()
  {
    ...
    if ($h3 > 0) {
      ...
      $this->db->update('master_jual');
    } else {
      $this->db->where('nofak_jual', $nofak);
      $this->db->delete('master_jual');
    }
  }
  ...
  function cetak_transaksi()
  {
    ...
    $x['data'] = $this->m_transaksi->get_laporan_transaksi($id_user, $akses);
